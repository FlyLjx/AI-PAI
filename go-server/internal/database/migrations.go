...
func EnsureSchema(db *sql.DB) error {
	...
	indexes := []struct {
		...
		{"idx_users_status_created_at", `CREATE INDEX idx_users_status_created_at ON users (status, created_at)`},
		{"idx_users_created_id", `CREATE INDEX idx_users_created_id ON users (created_at, id)`},
		{"idx_users_status_created_id", `CREATE INDEX idx_users_status_created_id ON users (status, created_at, id)`},
...
func schemaBootstrapStatements() []string {
	if CurrentDialect() == DialectPostgres {
		return []string{
			...
			`CREATE INDEX IF NOT EXISTS idx_api_access_keys_user_deleted_created ON api_access_keys (user_id, deleted_at, created_at, id)`,
