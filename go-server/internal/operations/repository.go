...
func (r *Repository) dashboardLegacy(ctx context.Context) (map[string]any, error) {
	...
	result["pending"] = map[string]any{
		...
		"recentFailedTasks": recentFailed,
	}
...
func (r *Repository) dashboardFast(ctx context.Context) (map[string]any, error) {
	...
	var taskTotalImages, runningTasks, recentFailed int
	var lastTask sql.NullTime
	taskStatRows, err := r.db.QueryContext(ctx, `
		SELECT
			status,
	...
	for taskStatRows.Next() {
		var status string
		var total, images, todayTotal, yesterdayTotal, recentTotal, runningTotal int
		...
		if err := taskStatRows.Scan(&status, &total, &images, &todayTotal, &yesterdayTotal, &recentTotal, &runningTotal, &countedFailureTotal, &excludedTotal, &todayCountedFailureTotal, &yesterdayCountedFailureTotal, &todayExcludedTotal, &yesterdayExcludedTotal, &lastCreatedAt); err != nil {
			taskStatRows.Close()
			return nil, err
		}
		taskTotal += total
...
func (r *Repository) CompleteOrder(ctx context.Context, outTradeNo string, tradeNo string, rebateConfigs ...InviteRechargeRebateConfig) (*RechargeOrder, bool, error) {
	...
	order.Credits = normalizeOperationCreditAmount(order.Credits)
	if !isSubscription && order.Credits <= 0 {
		return nil, false, ErrInvalidRechargeOrder
	}
	var balance float64
	...
	now := time.Now()
	creditRemark := "支付宝充值 " + order.OutTradeNo
	if isSubscription {
		var planName string
		var durationDays int
		if err := tx.QueryRowContext(ctx, `
			...
			FROM subscription_plans
			WHERE id=?
		`, strings.TrimSpace(*order.SubscriptionPlanID)).Scan(&planName, &durationDays); err != nil {
			return nil, false, err
		}
		if err := grantSubscriptionInTx(ctx, tx, order.UserID, *order.SubscriptionPlanID, durationDays, now); err != nil {
			return nil, false, err
		}
		creditRemark = "订阅套餐赠送 " + planName
	}
	if order.Credits > 0 {
		nextBalance := normalizeOperationCreditAmount(balance + order.Credits)
		if _, err := tx.ExecContext(ctx, `UPDATE users SET credits=? WHERE id=?`, nextBalance, order.UserID); err != nil {
			return nil, false, err
		}
		if _, err := tx.ExecContext(ctx, `
			INSERT INTO credit_logs (id, user_id, type, amount, balance_after, remark)
			VALUES (?, ?, 'recharge', ?, ?, ?)
		`, newOperationID(), order.UserID, order.Credits, nextBalance, creditRemark); err != nil {
			return nil, false, err
		}
	}
	...
	paidAt := appclock.DatabaseTime(now).Format(time.RFC3339)
	order.PaidAt = &paidAt
