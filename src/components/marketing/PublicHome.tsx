    ...
    const syncSession = () => {
      ...
      if (user) {
        setDestination({
          ...
          label: user.role === 'admin' ? '进入管理后台' : '进入控制台',
        });
      } else {
...
export function PublicHome() {
  const { destination, sessionReady } = useHomeDestination();
  const billingHref = sessionReady && destination.href !== '/register' ? '/billing' : '/register';
  const docsHref = sessionReady && destination.href !== '/register' ? '/docs' : '/register';
      ...
      <header className="landing-nav">
        <div className="landing-container landing-nav-inner">
          ...
          <div className="landing-auth">
            {sessionReady && destination.href !== '/register' ? (
              ...
              <>
                ...
                <Link className="btn primary landing-nav-cta" href="/register">注册</Link>
              </>
            )}
          </div>
        </div>
        ...
        <section className="landing-section landing-access-section" id="access">
          <div className="landing-container">
            ...
            <div className="landing-access-action">
              <Link className="btn primary" href={docsHref}>{sessionReady && destination.href !== '/register' ? '打开 API 文档' : '注册后查看文档'} <ArrowRight size={14} /></Link>
            </div>
          </div>
        </section>
