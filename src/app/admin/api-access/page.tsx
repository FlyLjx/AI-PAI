...
import { Activity, Check, KeyRound, Loader2, RefreshCw, Trash2 } from 'lucide-react';
...
import { type APIKey, type UsageLog, portalApi } from '@/lib/portal-api';
...
export default function AdminAPIAccessPage() {
  ...
  const saveConcurrency = async (key: APIKey) => {
    ...
    if (value < 1 || value > 50) return toast.error('并发上限须在 1 到 50 之间');
    setActionId(key.id);
    try {
  ...
  const keyActions = (key: APIKey) => (
    <div className="flex items-center justify-end gap-1">
      <button type="button" onClick={() => void toggleKey(key)} disabled={actionId === key.id} className={`rounded px-2 py-1 text-[10px] font-semibold ${key.status === 'active' ? 'text-amber-700 hover:bg-amber-50' : 'text-emerald-700 hover:bg-emerald-50'} disabled:opacity-40`}>{key.status === 'active' ? '禁用' : '启用'}</button>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        ...
        ].map(([label, value, note]) => <div key={String(label)} className="rounded-md border border-[#DCE4DF] bg-white p-3.5"><span className="text-[10px] font-semibold text-zinc-500">{label}</span><strong className="mt-1.5 block text-xl">{Number(value || 0).toLocaleString('zh-CN')}</strong><small className="mt-1 block text-[10px] text-zinc-400">{note}</small></div>)}
      </div>
      ...
      {tab === 'keys' && (keysLoading ? (
        ...
        <DataTable
          ...
          filterControls={<><select value={keyStatus} onChange={(event) => setKeyStatus(event.target.value)} className="h-8 rounded-md border border-[#DCE4DF] bg-white px-2 text-xs"><option value="all">全部状态</option><option value="active">已启用</option><option value="disabled">已禁用</option></select><span className="text-[10px] text-zinc-400">{filteredKeys.length} 条</span></>}
          ...
          renderRow={(key) => (
            <tr key={key.id} className="hover:bg-[#FAFBFA]">
              <td className="px-4 py-3"><strong className="block max-w-[190px] truncate font-medium">{key.userEmail || key.userId}</strong><small className="font-mono text-[9px] text-zinc-400">{key.userId}</small></td>
              <td className="px-4 py-3"><strong className="block max-w-[150px] truncate text-[11px] font-medium">{key.name}</strong><code className="text-[9px] text-zinc-400">{key.keyPrefix}••••••</code></td>
              <td className="px-4 py-3"><StatusBadge status={key.status === 'active' ? 'active' : 'disabled'} /></td>
              <td className="px-4 py-3"><div className="flex items-center gap-1"><input aria-label={`${key.name} 并发上限`} min={1} max={50} type="number" value={concurrencyDraft[key.id] ?? key.concurrencyLimit} onChange={(event) => setConcurrencyDraft((current) => ({ ...current, [key.id]: Number(event.target.value) }))} className="h-7 w-16 rounded border border-[#DCE4DF] px-2 font-mono text-[10px]" /><button type="button" onClick={() => void saveConcurrency(key)} disabled={actionId === key.id} title="保存并发" className="grid h-7 w-7 place-items-center rounded border border-[#86EFAC] bg-[#F0FDF4] text-[#047857] disabled:opacity-40"><Check className="h-3.5 w-3.5" /></button></div></td>
              <td className="px-4 py-3 font-mono text-[10px]"><span className="text-emerald-700">{key.successCount}</span> / <span className="text-red-600">{key.failedCount}</span><small className="mt-0.5 block text-[9px] text-zinc-400">共 {key.requestCount}</small></td>
          ...
          renderMobileItem={(key) => (
            <article key={key.id} className="rounded-md border border-[#DCE4DF] bg-white p-3.5">
              <div className="flex items-start justify-between gap-3"><div className="min-w-0"><strong className="block truncate text-sm">{key.name}</strong><small className="font-mono text-[9px] text-zinc-400">{key.keyPrefix}••••••</small></div><StatusBadge status={key.status === 'active' ? 'active' : 'disabled'} /></div>
              <p className="mt-2 truncate text-[10px] text-zinc-500">{key.userEmail || key.userId}</p>
              <div className="mt-3 grid grid-cols-3 divide-x divide-[#EDF0EE] border-y border-[#EDF0EE] py-2 text-center"><span><small className="block text-[9px] text-zinc-400">请求</small><strong className="text-[11px]">{key.requestCount}</strong></span><span><small className="block text-[9px] text-zinc-400">图片</small><strong className="text-[11px]">{key.imageCount}</strong></span><span><small className="block text-[9px] text-zinc-400">并发</small><strong className="text-[11px]">{key.concurrencyLimit}</strong></span></div>
      ...
      {tab === 'logs' && (logsLoading ? (
        ...
        <DataTable
          ...
          filterControls={<><select value={logStatusFilter} onChange={(event) => setLogStatusFilter(event.target.value)} className="h-8 rounded-md border border-[#DCE4DF] bg-white px-2 text-xs"><option value="all">全部状态</option><option value="queued">排队中</option><option value="processing">处理中</option><option value="success">成功</option><option value="failed">失败</option></select><span className="text-[10px] text-zinc-400">本页 {filteredLogs.length} 条</span></>}
          currentPage={logPage}
          ...
          renderRow={(log) => { const status = logStatus(log.status); return (
            <tr key={log.id} className="hover:bg-[#FAFBFA]">
              ...
              <td className="px-4 py-3"><strong className="block max-w-[170px] truncate font-medium">{log.userEmail || log.userId}</strong><small className="block max-w-[150px] truncate text-[9px] text-zinc-400">{log.keyName || log.keyPrefix || '-'}</small></td>
              <td className="max-w-[150px] truncate px-4 py-3 font-mono text-[10px]">{log.endpoint}</td>
              <td className="max-w-[160px] truncate px-4 py-3">{log.model || '-'}</td>
              <td className="px-4 py-3 text-[10px] text-zinc-500">{log.size || '-'} · {log.quality || '-'} · {log.imageCount || log.quantity || 0} 张</td>
              <td className="px-4 py-3"><StatusBadge status={status.badge} customLabel={status.label} /></td>
              <td className="max-w-[220px] truncate px-4 py-3 text-[10px] text-red-600" title={log.errorMessage || log.prompt || ''}>{log.errorMessage || '-'}</td>
            </tr>
          ); }}
          renderMobileItem={(log) => { const status = logStatus(log.status); return (
            <article key={log.id} className="rounded-md border border-[#DCE4DF] bg-white p-3.5">
              <div className="flex items-start justify-between gap-3"><div className="min-w-0"><strong className="block truncate text-sm">{log.model || log.endpoint}</strong><small className="block truncate text-[9px] text-zinc-400">{log.userEmail || log.userId} · {log.keyName || log.keyPrefix || '-'}</small></div><StatusBadge status={status.badge} customLabel={status.label} /></div>
              <p className="mt-3 truncate rounded bg-[#F6F8F6] px-2 py-1.5 font-mono text-[10px]">{log.endpoint}</p>
              <div className="mt-2 flex items-center justify-between text-[9px] text-zinc-400"><span>{log.size || '-'} · {log.imageCount || log.quantity || 0} 张</span><span>{formatDate(log.createdAt)}</span></div>
              {log.errorMessage && <p className="mt-2 line-clamp-2 text-[10px] text-red-600">{log.errorMessage}</p>}
            </article>
          ); }}
        />
      ))}
