...
export async function login(email: string, password: string): Promise<PortalUser> {
  ...
  if (data.role === 'admin') {
    const admin = await api<{ token: string }>('/api/admin/login', {
      method: 'POST', body: JSON.stringify({ email, password }),
    });
    adminToken = admin.data.token;
  }
  return saveSession({ ...data, credits: Number(data.credits || 0), adminToken });
}
...
export async function refreshSession(user = getSession()): Promise<PortalUser> {
  ...
  if (user.role === 'admin' && user.adminToken) {
    await api('/api/admin/session', {}, user.adminToken);
  }
  const { data } = await api<PortalUser>(`/api/users/${encodeURIComponent(user.id)}/profile`, {}, user.token);
  return saveSession({ ...user, ...data, token: user.token, adminToken: user.adminToken, credits: Number(data.credits || 0) });
}
...
export const portalApi = {
  ...
  dashboard: () => api<Record<string, unknown>>('/api/dashboard?limit=8', {}, adminToken()),
  users: () => api<PortalUser[]>('/api/users', {}, adminToken()),
  createUser: (input: Record<string, unknown>) => api<PortalUser>('/api/users', { method: 'POST', body: JSON.stringify(input) }, adminToken()),
  updateUser: (id: string, input: Record<string, unknown>) => api<PortalUser>(`/api/users/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(input) }, adminToken()),
  deleteUser: (id: string) => api(`/api/users/${encodeURIComponent(id)}`, { method: 'DELETE' }, adminToken()),
  grantSubscription: (id: string, input: Record<string, unknown>) => api(`/api/users/${encodeURIComponent(id)}/subscription`, { method: 'POST', body: JSON.stringify(input) }, adminToken()),
  providers: () => api<Record<string, unknown>[]>('/api/api-providers', {}, adminToken()),
  createProvider: (input: Record<string, unknown>) => api('/api/api-providers', { method: 'POST', body: JSON.stringify(input) }, adminToken()),
  updateProvider: (id: string, input: Record<string, unknown>) => api(`/api/api-providers/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(input) }, adminToken()),
  deleteProvider: (id: string) => api(`/api/api-providers/${encodeURIComponent(id)}`, { method: 'DELETE' }, adminToken()),
  testProvider: (id: string) => api(`/api/api-providers/${encodeURIComponent(id)}/test`, { method: 'POST' }, adminToken()),
  models: () => api<Record<string, unknown>[]>('/api/models', {}, adminToken()),
  createModel: (input: Record<string, unknown>) => api('/api/models', { method: 'POST', body: JSON.stringify(input) }, adminToken()),
  updateModel: (id: string, input: Record<string, unknown>) => api(`/api/models/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(input) }, adminToken()),
  deleteModel: (id: string) => api(`/api/models/${encodeURIComponent(id)}`, { method: 'DELETE' }, adminToken()),
  adminPlans: () => api<Plan[]>('/api/subscriptions/plans', {}, adminToken()),
  createPlan: (input: Partial<Plan>) => api<Plan>('/api/subscriptions/plans', { method: 'POST', body: JSON.stringify(input) }, adminToken()),
  updatePlan: (id: string, input: Partial<Plan>) => api<Plan>(`/api/subscriptions/plans/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(input) }, adminToken()),
  deletePlan: (id: string) => api(`/api/subscriptions/plans/${encodeURIComponent(id)}`, { method: 'DELETE' }, adminToken()),
  recharges: (page = 1) => api<Record<string, unknown>[]>(`/api/recharge/orders${query({ page, pageSize: 30 })}`, {}, adminToken()),
  adminKeys: () => api<{ items: APIKey[]; stats: Record<string, number> }>('/api/admin/api-access/keys', {}, adminToken()),
  updateAdminKey: (id: string, input: { status?: string; concurrencyLimit?: number }) => api<APIKey>(`/api/admin/api-access/keys/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(input) }, adminToken()),
  deleteAdminKey: (id: string) => api(`/api/admin/api-access/keys/${encodeURIComponent(id)}`, { method: 'DELETE' }, adminToken()),
  adminUsage: (page = 1) => api<UsageLog[]>(`/api/admin/api-access/logs${query({ page, pageSize: 30 })}`, {}, adminToken()),
  tasks: (page = 1) => api<Record<string, unknown>[]>(`/api/tasks${query({ page, pageSize: 30 })}`, {}, adminToken()),
  settings: () => api<Record<string, unknown>>('/api/settings', {}, adminToken()),
  updateSettings: (input: Record<string, unknown>) => api('/api/settings', { method: 'PATCH', body: JSON.stringify(input) }, adminToken()),
  logs: () => api<SystemLogFile[]>('/api/system-logs', {}, adminToken()),
  systemLogDetail: (name: string, maxBytes = 300000) => api<SystemLogDetail>(`/api/system-logs/detail${query({ name, maxBytes })}`, {}, adminToken()),
  deleteSystemLog: (name: string) => api(`/api/system-logs/${encodeURIComponent(name)}`, { method: 'DELETE' }, adminToken()),
};
