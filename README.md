...
- 图片模型支持 `/v1/chat/completions`，兼容 new-api 默认渠道测试并返回 Markdown 图片内容
...
宿主机更新 worker 收到请求后会拉取同一个 `build-N` 的 Web、Admin 和 API 镜像，直接替换应用容器。更新命令使用 `--no-deps`，不会备份、重建或替换 PostgreSQL 容器；健康检查失败时自动恢复更新前的应用镜像。
