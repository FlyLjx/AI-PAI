...
export default function AdminPackagesPage() {
  ...
  const rowActions = (plan: AdminPlan) => (
    <div className="flex items-center justify-end gap-1">
      ...
      <button type="button" onClick={() => void toggleStatus(plan)} disabled={actionId === plan.id} className={`rounded px-2 py-1 text-[11px] font-semibold ${plan.status === 'active' ? 'text-amber-700 hover:bg-amber-50' : 'text-emerald-700 hover:bg-emerald-50'} disabled:opacity-40`}>{plan.status === 'active' ? '下架' : '上架'}</button>
        ...
        <DataTable
          ...
          renderRow={(plan) => (
            <tr key={plan.id} className="hover:bg-[#FAFBFA]">
              <td className="px-4 py-3"><span className="flex items-center gap-2"><strong className="block max-w-[170px] truncate font-medium">{plan.name}</strong>{plan.badge && <span className="rounded border border-amber-200 bg-amber-50 px-1.5 py-0.5 text-[10px] font-semibold text-amber-800">{plan.badge}</span>}</span><small className="mt-0.5 block max-w-[230px] truncate text-[10px] text-zinc-400">{plan.description || '无说明'}</small></td>
              ...
              <td className="px-4 py-3 text-[11px] text-zinc-500">{plan.allowedModelIds?.length ? `${plan.allowedModelIds.length} 个模型` : plan.allowedProviderIds?.length ? `${plan.allowedProviderIds.length} 个上游` : '全部模型'}</td>
          ...
          renderMobileItem={(plan) => (
            <article key={plan.id} className="rounded-md border border-[#DCE4DF] bg-white p-3.5">
              <div className="flex items-start justify-between gap-3"><div className="min-w-0"><strong className="block truncate text-sm">{plan.name}</strong><small className="line-clamp-1 text-[10px] text-zinc-400">{plan.description || '无说明'}</small></div><StatusBadge status={plan.status === 'active' ? 'active' : 'disabled'} /></div>
              <div className="mt-3 grid grid-cols-3 divide-x divide-[#EDF0EE] border-y border-[#EDF0EE] py-2 text-center"><span><small className="block text-[10px] text-zinc-400">售价</small><strong className="text-[12px] text-[#047857]">{formatCNY(Number(plan.amount || 0))}</strong></span><span><small className="block text-[10px] text-zinc-400">有效期</small><strong className="text-[12px]">{plan.durationDays} 天</strong></span><span><small className="block text-[10px] text-zinc-400">额度</small><strong className="text-[12px]">{plan.quotaImages} 张</strong></span></div>
      ...
      {editorOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/40 p-4 sm:grid sm:place-items-center">
          <form onSubmit={savePlan} className="mx-auto w-full max-w-3xl overflow-hidden rounded-md border border-[#DCE4DF] bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-[#DCE4DF] px-5 py-3.5"><div><h2 className="text-sm font-semibold">{editing ? '编辑订阅套餐' : '新增订阅套餐'}</h2><p className="mt-0.5 text-[11px] text-zinc-500">套餐用于周期额度计费，与余额按量计费并行。</p></div><button type="button" onClick={() => setEditorOpen(false)} className="rounded p-1 text-zinc-500 hover:bg-zinc-100"><X className="h-4 w-4" /></button></div>
            <div className="space-y-5 p-5">
              <section className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <label><span className="mb-1 block text-[11px] font-semibold text-zinc-500">套餐名称</span><input required value={draft.name} onChange={(event) => updateDraft('name', event.target.value)} className="w-full rounded-md border border-[#DCE4DF] px-3 py-2 text-xs outline-none focus:border-[#12B76A]" /></label>
                <label><span className="mb-1 block text-[11px] font-semibold text-zinc-500">展示标签</span><input value={draft.badge} onChange={(event) => updateDraft('badge', event.target.value)} placeholder="如：推荐" className="w-full rounded-md border border-[#DCE4DF] px-3 py-2 text-xs outline-none focus:border-[#12B76A]" /></label>
              ...
              <section className="grid grid-cols-2 gap-3 border-t border-[#DCE4DF] pt-4 sm:grid-cols-4">
                <label><span className="mb-1 block text-[11px] font-semibold text-zinc-500">售价（元）</span><input min={0} step="0.01" type="number" value={draft.amount} onChange={(event) => updateDraft('amount', Number(event.target.value))} className="w-full rounded-md border border-[#DCE4DF] px-2 py-2 font-mono text-xs" /></label>
                <label><span className="mb-1 block text-[11px] font-semibold text-zinc-500">有效期（天）</span><input min={1} type="number" value={draft.durationDays} onChange={(event) => updateDraft('durationDays', Number(event.target.value))} className="w-full rounded-md border border-[#DCE4DF] px-2 py-2 font-mono text-xs" /></label>
                <label><span className="mb-1 block text-[11px] font-semibold text-zinc-500">图片额度</span><input min={1} type="number" value={draft.quotaImages} onChange={(event) => updateDraft('quotaImages', Number(event.target.value))} className="w-full rounded-md border border-[#DCE4DF] px-2 py-2 font-mono text-xs" /></label>
                <label><span className="mb-1 block text-[11px] font-semibold text-zinc-500">排序</span><input min={0} type="number" value={draft.sortOrder} onChange={(event) => updateDraft('sortOrder', Number(event.target.value))} className="w-full rounded-md border border-[#DCE4DF] px-2 py-2 font-mono text-xs" /></label>
              </section>
              <section className="grid grid-cols-1 gap-4 border-t border-[#DCE4DF] pt-4 sm:grid-cols-2">
                <div><div className="mb-2 flex items-center justify-between"><span className="text-[11px] font-semibold text-zinc-500">限定上游</span><button type="button" onClick={() => updateDraft('allowedProviderIds', [])} className="text-[10px] text-[#047857]">清空即全部</button></div><div className="max-h-36 space-y-1 overflow-y-auto rounded-md border border-[#DCE4DF] p-2">{providers.map((provider) => <label key={provider.id} className="flex items-center gap-2 rounded px-2 py-1.5 text-[11px] hover:bg-[#F6F8F6]"><input type="checkbox" checked={draft.allowedProviderIds.includes(provider.id)} onChange={() => toggleArrayValue('allowedProviderIds', provider.id)} className="accent-[#047857]" /><span className="truncate">{provider.name}</span></label>)}{!providers.length && <p className="py-4 text-center text-[11px] text-zinc-400">暂无上游</p>}</div></div>
                <div><div className="mb-2 flex items-center justify-between"><span className="text-[11px] font-semibold text-zinc-500">限定模型</span><button type="button" onClick={() => updateDraft('allowedModelIds', [])} className="text-[10px] text-[#047857]">清空即全部</button></div><div className="max-h-36 space-y-1 overflow-y-auto rounded-md border border-[#DCE4DF] p-2">{models.map((model) => <label key={model.id} className="flex items-center gap-2 rounded px-2 py-1.5 text-[11px] hover:bg-[#F6F8F6]"><input type="checkbox" checked={draft.allowedModelIds.includes(model.id)} onChange={() => toggleArrayValue('allowedModelIds', model.id)} className="accent-[#047857]" /><span className="truncate">{model.displayName || model.modelName || model.id}</span></label>)}{!models.length && <p className="py-4 text-center text-[11px] text-zinc-400">暂无模型</p>}</div></div>
              </section>
              <label className="border-t border-[#DCE4DF] pt-4 text-xs"><span className="mb-1 block"><strong className="block text-[11px]">套餐状态</strong><small className="text-[10px] text-zinc-400">下架后不再出现在用户购买列表</small></span><AppSelect value={draft.status} onValueChange={(value) => updateDraft('status', value as PlanDraft['status'])} ariaLabel="套餐状态" options={[{ value: 'active', label: '上架' }, { value: 'disabled', label: '下架' }]} /></label>
            </div>
