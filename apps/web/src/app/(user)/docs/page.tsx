...
const requestParameters: Record<Endpoint, APIParameter[]> = {
  ...
  generations: [
    ...
    { name: 'referenceImages', type: 'string[]', required: '否', description: '参考图 URL 或 data URI 数组，最多 4 张' },
  ...
  edits: [
    ...
    { name: 'image / images', type: 'file / file[]', required: '二选一', description: 'multipart 参考图，最多 4 张，单张最大 20MB' },
    { name: 'image_url', type: 'string / string[]', required: '二选一', description: 'JSON 请求中的参考图 URL 或 data URI，最多 4 张' },
...
const responseParameters: Record<Endpoint, APIParameter[]> = {
  ...
  generations: [
    ...
    { name: 'data[].url', type: 'string', required: '-', description: 'response_format=url 时的图片地址' },
    { name: 'data[].b64_json', type: 'string', required: '-', description: 'response_format=b64_json 时的 Base64 图片数据' },
    { name: 'usage', type: 'object', required: '-', description: '上游报告的图片 Token 用量；部分上游为估算值，未提供时省略' },
  ],
  edits: [
    { name: 'created', type: 'integer', required: '-', description: '响应创建时间，Unix 秒级时间戳' },
    { name: 'data[].url', type: 'string', required: '-', description: 'response_format=url 时的图片地址' },
